...
export const getReservations = async () => {
  ...
  const r = await db.reservation.findMany({ where: {userId: session?.user.id!}, include: { restaurant: true}});
...
export const getReservationByIdWithReviews = async (id: ReservationId) => {
  ...
  const r = await db.reservation.findFirst({
    where: { id: reservationId, userId: session?.user.id!},
    include: { restaurant: { include: {reservations: true } }, reviews: { include: {reservation: true } } }
  });
