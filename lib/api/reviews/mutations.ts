...
export const updateReview = async (id: ReviewId, review: UpdateReviewParams) => {
  ...
  try {
    const r = await db.review.update({ where: { id: reviewId, userId: session?.user.id! }, data: newReview})
    return { review: r };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
...
export const deleteReview = async (id: ReviewId) => {
  ...
  const { id: reviewId } = reviewIdSchema.parse({ id });
  try {
    const r = await db.review.delete({ where: { id: reviewId, userId: session?.user.id! }})
    return { review: r };
  } catch (err) {
    const message = (err as Error).message ?? "Error, please try again";
    console.error(message);
