...
export const insertReservationParams = baseSchema.extend({
...
export const updateReservationParams = updateReservationSchema.extend({
  occupants: z.coerce.number(),
  reservationDate: z.coerce.date(),
  restaurantId: z.coerce.string().min(1)
}).omit({ 
...
export type CompleteReservation = Awaited<ReturnType<typeof getReservations>>["reservations"][number];
