...
const ReservationForm = ({
  ...
  const { errors, hasErrors, setErrors, handleChange } = useValidatedForm<Reservation>(
  ...
  const onSuccess = (action: Action, data?: { error: string; values: Reservation }) => {
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const reservationParsed = await insertReservationParams.safeParseAsync({
      restaurantId,
      ...payload,
    })
  ...
  return (
    <form action={handleSubmit} onChange={handleChange} className={'space-y-8'}>
      ...
      <div>
        <Label
          className={cn('mb-2 inline-block', errors?.status ? 'text-destructive' : '')}
        >
          Status
        </Label>
        <Input
            ...
            <SelectContent>
              {restaurants?.map((restaurant) => (
                <SelectItem key={restaurant.id} value={restaurant.id.toString()}>
                  {restaurant.name}
                  {/* TODO: Replace with a field from the restaurant model */}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
